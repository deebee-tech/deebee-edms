...
export function active(node: HTMLAnchorElement, opts: Omit<Options, "url"> = {}) {
	...
	$effect(() => {
		...
		untrack(() => {
			node.setAttribute("data-active", checkIsActive(node.href, { ...opts, url: page.url }).toString());
		});
	});
}
...
export const checkIsActive = (
	...
	const isParentRoute: boolean =
		(activeForSubdirectories == undefined || activeForSubdirectories) && url.pathname.startsWith(href ?? "");
