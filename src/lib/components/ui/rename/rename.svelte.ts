...
class RenameInputState {
	...
	async startEditing(selection?: { start: number; end?: number }) {
		...
		if (selection !== undefined) {
			this.opts.inputRef.current?.setSelectionRange(selection.start, selection.end ?? selection.start);
		} else {
			if (this.opts.fallbackSelectionBehavior.current === "start") {
				...
				this.opts.inputRef.current?.setSelectionRange(this.editingValue.length, this.editingValue.length);
