<script lang="ts">
	...
	import { getOperatorDescriptor, getOperatorsForType } from "./filter-operators.js";
	import {
		FilterOperator,
		type ColumnFilter,
		type ColumnMeta,
		...
		<div class="max-h-96 space-y-4 overflow-y-auto py-4">
			{#each groups as group, groupIdx (group.id)}
				...
				<div class="rounded-lg border p-3">
					<div class="mb-2 flex items-center justify-between">
						<div class="flex items-center gap-2">
							...
							<Select.Root
								...
								value={group.logic}
								onValueChange={(v) => {
									if (v) group.logic = v as FilterGroupLogic;
								}}
							>
					...
					<div class="grid gap-x-2 gap-y-2" style="grid-template-columns: 2fr 1.5fr 2fr auto;">
						{#each group.rows as row, rowIdx (row.id)}
							...
							<Select.Root
								type="single"
								value={row.column}
								onValueChange={(v) => handleColumnChange(groupIdx, rowIdx, v)}
							>
								<Select.Trigger class="h-8 w-full text-xs">
									<span class="truncate">{col?.label ?? "Column"}</span>
								</Select.Trigger>
								<Select.Content>
							...
							<Select.Root
								...
								value={row.operator}
								onValueChange={(v) => {
									if (v) row.operator = v as FilterOperator;
								}}
							>
								<Select.Trigger class="h-8 w-full text-xs">
									<span class="truncate">{desc?.label ?? "Operator"}</span>
								</Select.Trigger>
								<Select.Content>
							...
							<div class="flex items-center gap-2">
								{#if desc && desc.valueCount > 0}
									<Input class="h-8 flex-1 text-xs" placeholder="Value..." bind:value={row.value} />
									{#if desc.valueCount === 2}
										<Input class="h-8 flex-1 text-xs" placeholder="To..." bind:value={row.valueTo} />
									{/if}
								{:else}
									<div></div>
								{/if}
