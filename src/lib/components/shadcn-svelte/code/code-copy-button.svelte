<script lang="ts">
	import { useCodeCopyButton } from "$lib/components/shadcn-svelte/code/code.svelte.js";
	import type { CodeCopyButtonProps } from "$lib/components/shadcn-svelte/code/types.js";
	import { CopyButton } from "$lib/components/shadcn-svelte/copy-button";
	import { cn } from "$lib/utils.js";

	let {
		ref = $bindable(null),
		variant = "ghost",
		size = "icon",
		class: className,
		...rest
	}: CodeCopyButtonProps = $props();

	const copyButton = useCodeCopyButton();
</script>

<CopyButton
	bind:ref
	class={cn("absolute top-2 right-2", className)}
	text={copyButton.code}
	{variant}
	{size}
	{...rest}
/>
