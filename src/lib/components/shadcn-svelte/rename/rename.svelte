...
<script lang="ts" generics="TagName extends TextElementTagNames">
	import { useRenameInput } from "$lib/components/shadcn-svelte/rename/rename.svelte.js";
	import type { TextElementTagNames } from "$lib/components/shadcn-svelte/rename/types.js";
...
{#if mode === "edit"}
	...
	{:else}
		<input bind:this={inputRef} type="text" autocomplete="off" bind:value={rootState.editingValue} {...inputProps} />
	{/if}
{:else if mode === "view"}
