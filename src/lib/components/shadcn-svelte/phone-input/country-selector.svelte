<script lang="ts">
	...
	import * as Command from "$lib/components/shadcn-svelte/command";
	import Flag from "$lib/components/shadcn-svelte/phone-input/flag.svelte";
	import * as Popover from "$lib/components/shadcn-svelte/popover";
	...
	<Popover.Trigger>
		{#snippet child({ props })}
			...
			>
				...
				<ChevronsUpDownIcon class={cn("-mr-2 h-4 w-4 opacity-50", disabled ? "hidden" : "opacity-100")} />
			</Button>
		{/snippet}
	</Popover.Trigger>
	<Popover.Content
				...
				<ScrollArea class="h-72">
					...
					<Command.Group>
						{#each countries.sort(order) as country (country.id)}
							<Command.Item class="gap-2" value={country.name} onSelect={() => selectCountry(country)}>
								<Flag {country} />
